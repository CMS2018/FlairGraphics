...
namespace FlairGraphic.Base.Models
{
        ...
        public override int SaveChanges()
        {
            ...
            try
            {
                foreach (var entry in entitiesNew)
                {
                    ...
                    if (entity != null)
                    {
                        Int32 systemID = Convert.ToInt32(STUtil.GetWebConfigValue("SYSTEM_ID"));
                        Int32 companyID = Convert.ToInt32(STUtil.GetWebConfigValue("COMPANY_ID"));

                        if (STUtil.IsAuthenticated())
                        {
                            if (STUtil.GetSessionValue(UserInfo.IsCompanyAddUpdate.ToString()) == "1")
                            {

                                STUtil.SetProperty(entity, "company_id", STUtil.IsAuthenticated() ? SessionUtil.GetCompanyID() : companyID);

                            }
                            STUtil.SetProperty(entity, "is_active", true);
                            STUtil.SetProperty(entity, "is_login_enable", true);
                            STUtil.SetProperty(entity, "created_by", STUtil.IsAuthenticated() ? SessionUtil.GetUserID() : systemID);
                            STUtil.SetProperty(entity, "created_date", STUtil.GetCurrentDateTime());
                ...
                foreach (var entry in entitiesModified)
                {
                    ...
                    if (entity != null)
                    {
                        ...
                        if (STUtil.IsAuthenticated())
                        {
                            ...
                            STUtil.SetProperty(entity, "updated_by", STUtil.IsAuthenticated() ? SessionUtil.GetUserID() : systemID);
                            STUtil.SetProperty(entity, "updated_date", STUtil.GetCurrentDateTime());
                        }
                    }
                }
    ...
    public static class AWSUtil
    {
        ...
        public static string GetFileURL(string filename ,string CompanyFolderName="")
        {
            ...
            var s3Path = STUtil.GetWebConfigValue("ImagePath");
            var SystemImagePath = STUtil.GetWebConfigValue("SystemImagePath");
              CompanyFolderName =string.IsNullOrEmpty(CompanyFolderName)? STUtil.GetSessionValue(UserInfo.CompanyFolderName.ToString()): CompanyFolderName;
            var NA_IMAGE = STUtil.GetWebConfigValue("NA_IMAGE");
            return (!NA_IMAGE.Contains(filename) ? s3Path + CompanyFolderName + filename : SystemImagePath+"/"+ filename);
        }
        public static void FolderCreation(string AWSProfileName, string company_folder_name)
        {
            string root = HttpContext.Current.Server.MapPath(AWSProfileName);
            string subdir = HttpContext.Current.Server.MapPath(AWSProfileName + company_folder_name);
    ...
    public static class STUtil
    {
        ...
        public static string GetImagePath(string img, int ? gender = 0)
        {
            ...
            int companyid = SessionUtil.GetCompanyID();
            var SNAG_PATH = STUtil.GetWebConfigValue("ImagePath");
            var CompanyFolderName = STUtil.GetSessionValue(UserInfo.CompanyFolderName.ToString());
            var NA_IMAGE = STUtil.GetWebConfigValue("NA_IMAGE");
            var DefaultImgs = "";
            ...
            try
            {
                ...
                foreach (var item in authlevels)
                {
                    var au = (from R in db.roles.AsEnumerable()
                              join RA in db.role_action.AsEnumerable() on R.role_id equals RA.role_id
                              where R.role_bit == item && R.company_id == SessionUtil.GetCompanyID()
                              select new
                              {
        ...
        public static string UserLoginPolicy(int UserID, Boolean isFaild)
        {
            ...
            try
            {
                ...
                if (isFaild)
                {
                    objTable.password_failed_attempt = objTable.password_failed_attempt + 1;
                    Int32 No_failed_attempt_left = No_failed_attempt - objTable.password_failed_attempt;

                    result = "Invalid Login ID or Password. You have only  " + No_failed_attempt_left + " attempt left.";

                    if (objTable.password_failed_attempt > No_failed_attempt)
                    {
                        objTable.is_account_locked = true;
                ...
                {
                    objTable.password_failed_attempt = 0;
                    objTable.is_account_locked = false;
                    objTable.last_login_date = GetCurrentDateTime();
                }
                db.Entry(objTable).State = EntityState.Modified;
                db.SaveChanges();
        ...
        public static int CheckLoginIDExist(String LoginID)
        {
            ...
            return info.Length == 2 ? db.users.AsEnumerable().Where(x => (x.login_id ?? "").Trim().ToUpper() == login_id.Trim().ToUpper() && x.user_id != Convert.ToInt32(user_id)).Count() :
                db.users.AsEnumerable().Where(x => x.login_id.Trim().ToUpper() == login_id.Trim().ToUpper()).Count();
        }
        public static int CheckMobileExist(String id)
        {
            ...
            var userId = info[1];
            Int32 companyID = SessionUtil.GetCompanyID();
            return info.Length == 2 ? db.users.AsEnumerable().Where(x => x.mobile.Trim().ToUpper() == mobileNo.Trim().ToUpper() && x.user_id != Convert.ToInt32(userId) && x.company_id == companyID).Count() :
        ...
        public static int CheckEmailIDExist(String id)
        {
            ...
            Int32 companyID = SessionUtil.GetCompanyID();
            return info.Length == 2 ? db.users.AsEnumerable().Where(x => x.email_id.Trim().ToUpper() == email_id.Trim().ToUpper() && x.user_id != user_id && x.company_id == companyID).Count() :
                db.users.AsEnumerable().Where(x => x.email_id.Trim().ToUpper() == email_id.Trim().ToUpper()).Count();
        }

        public static int CheckEmployeeCodeExist(string login_id)
        {
            return (from M in db.users.AsEnumerable()
                    where (M.login_id.Trim().ToUpper() == login_id.Trim().ToUpper())
                    select M).Count();
        }
        #endregion
        ...
        {
            var listAll = (from M in GetControllerActionNames(GetType("FlairGraphic.Controllers." + controllerName)).AsEnumerable()
                           orderby M
                           select new SelectListItem
                           {
                               Value = M,
        ...
        public static List<SelectListItem> GetListActionAssignedByRoleAndController(int roleID, string controllerName)
        {
            var listAssigned = (from r in db.role_action
                                where r.role_id == roleID && r.controller_name == controllerName
        ...
        public static String getUserByRoleBit(int roleBit = 0)
        {
            ...
            if (roleBit > 0)
            {
                var roleList = db.roles.AsEnumerable().Where(x => x.role_bit == roleBit).ToList();

                userName = roleList.Count > 0 ? roleList.FirstOrDefault().role_name : "";
            }
            return userName;
        }
        #endregion
        ...
        #endregion
        public static string DeleteMaster(String TableName, Int32 PrimaryKeyID)
        {
            string result = string.Empty;
            try
            {
                ...
                {
                    case "country":
                        country country = db.countries.Find(PrimaryKeyID);
                        if (country == null)
                        {
                    ...
                    case "state":
                        ...
                        state state = db.states.Find(PrimaryKeyID);
                        if (state == null)
                        {
                            result = "No Record Found!";
                    ...
                    case "users":
                        user user = db.users.Find(PrimaryKeyID);
                        if (user == null)
                        {
                            result = "No Record Found!";
                    ...
                    case "company":
                        company company = db.companies.Find(PrimaryKeyID);
                        if (company == null)
                        {
                            result = "No Record Found!";
                        }
                        ...
                        break;
                    //case "Pincode":
                    //    pincode pincode = db.pincodes.Find(PrimaryKeyID);
                    //    if (pincode == null)
                    //    {
                    //        result = "No Record Found!";
                    //    }
                    ...
                    //    break;
                    //case "area":
                    //    area area = db.areas.Find(PrimaryKeyID);
                    //    if (area == null)
                    //    {
                    //        result = "No Record Found!";
                    //    }
                    ...
                    //    break;
                    //case "employee_doc":
                    //    employee_doc employee_doc = db.employee_doc.Find(PrimaryKeyID);
                    //    if (employee_doc == null)
                    //    {
                    //        result = "No Record Found!";
                    //    }
                    //    else
                    //    {
                    //        db.employee_doc.Remove(employee_doc);
                    //        db.SaveChanges();
                    //    }
                    //    break;
                }
            ...
            catch (Exception ex)
            {
                ...
                result = ((ex.InnerException).InnerException).Message.ToString();
            }
            return result;
        }
    ...
    public static class SessionUtil
    {
        ...
        {
            return Convert.ToInt32(STUtil.GetSessionValue(UserInfo.UserID.ToString()));
        }
        public static string GetFullName()
        {
            return STUtil.GetSessionValue(UserInfo.FullName.ToString());
        }

        public static string GetLoginID()
        {
            return STUtil.GetSessionValue(UserInfo.LoginID.ToString());
        }

        public static int GetRoleID()
        {
            return Convert.ToInt32(STUtil.GetSessionValue(UserInfo.RoleID.ToString()));
        }

        public static int GetRoleBit()
        {
            return Convert.ToInt32(STUtil.GetSessionValue(UserInfo.RoleBit.ToString()));
        }

        public static int GetRoleName()
        {
            return Convert.ToInt32(STUtil.GetSessionValue(UserInfo.RoleName.ToString()));
        }
        public static int GetCompanyID()
        {
            return Convert.ToInt32(STUtil.GetSessionValue(UserInfo.CompanyID.ToString()));
        }

        public static string GetCompanyFolderName()
        {
            return STUtil.GetSessionValue(UserInfo.CompanyFolderName.ToString());
        }

        public static string GetTimeZone()
        {
            return STUtil.GetSessionValue(UserInfo.time_zone.ToString());
        }
        public static string GetCurrencySymbol()
        {
            return STUtil.GetSessionValue(UserInfo.currency_symbol.ToString());
        }
        public static string GetDateFormatCSharp()
        {
            return STUtil.GetSessionValue(UserInfo.date_format_code_csharp.ToString());
        }
        public static string GetTimeFormatCSharp()
        {
            return STUtil.GetSessionValue(UserInfo.time_format_code_csharp.ToString());
        }
        public static string GetDateFormatJS()
        {
            return STUtil.GetSessionValue(UserInfo.date_format_code_js.ToString());
        }
        public static string GetTimeFormatJS()
        {
            return STUtil.GetSessionValue(UserInfo.time_format_code_js.ToString());
        }

        public static string GetServerTimeZone()
        {
            return STUtil.GetSessionValue(UserInfo.SERVER_TIME_ZONE.ToString());
        }

        public static int GetApplicationTypeID()
        {
            return Convert.ToInt32(STUtil.GetSessionValue(UserInfo.application_type_id.ToString()));
        }

        public static string GetApplicationTypeName()
        {
            return (STUtil.GetSessionValue(UserInfo.application_type_name.ToString()));
        }

        public static string GetSystemCompanyFolder()
        {
            return (STUtil.GetSessionValue(UserInfo.system_company_folder.ToString()));
        }
        public static string GetDefaultContactID()
        {
            return (STUtil.GetSessionValue(UserInfo.default_contact_id.ToString()));
        }
        public static string GetCompanyContractDocFileName()
        {
            return (STUtil.GetSessionValue(UserInfo.company_doc_file_name.ToString()));
        }
        ...
        {
            return Convert.ToBoolean(STUtil.GetSessionValue(UserInfo.is_service_provider.ToString()));
        }

        public static Boolean IsChangeRequester()
        {
            return Convert.ToBoolean(STUtil.GetSessionValue(UserInfo.is_change_requester.ToString()));
        }

        public static Int32 GetCreateWOAccessID()
        {
            return Convert.ToInt32(STUtil.GetSessionValue(UserInfo.create_work_order_access_id.ToString()));
        }
        public static Int32 GetViewWOAccessID()
        {
            return Convert.ToInt32(STUtil.GetSessionValue(UserInfo.view_work_order_access_id.ToString()));
        }
    }
    ...
    public class HtmlModel
    {
        ...
        private void Init()
        {
            ...
            switch (this.ControlType)
            {
                ...
                case ControlType.hidden:
                    this.ReadOnlyProp = "";
                    requiredAttr.Add("data-is-title", STUtil.LowerCase(this.IsTitle));
                    break;
                ...
                case ControlType.select:
                    ...
                    if (this.IsMultiSelect)
                    {
                        ...
                        requiredAttr.Add("data-values", STUtil.ListToJson(this.ListSelected));
        ...
        private string GetServerTask(ServerTask item)
        {
            ...
            string taskType = "";
            string async = STUtil.LowerCase(item.Async);
            string skipValidation = STUtil.LowerCase(item.SkipValidation);
    ...
    //                STUtil.CopyObject(u, find, parmIds);
    ...
    //                u.password = u.password != "" ? u.password : STUtil.GetRandomPasswordNumber(6);
    //                db.users.Add(u);
    ...
    public static class ExceptionLogging
    {
        ...
        public static void SendErrorToText(Exception ex)
        {
            ...
            try
            {
                string filepath = context.Current.Server.MapPath("~/Files//");  //Text File Path
